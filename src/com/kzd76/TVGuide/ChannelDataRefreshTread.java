...
public class ChannelDataRefreshTread extends Thread{
	...
	public ChannelDataRefreshTread(Handler handler, String[] channelIds, String threadName) {
	...
	public void run(){
		Log.d(Constants.LOG_MAIN_TAG + localLogTag, "Thread started, channels: " + channelIds.length);
		...
		for (String channelId : channelIds) {
			dayId = 1; // TODO Change to a variable if number of days for guide is handled
				...
				if (cd != null) {
					...
					for (ChannelEvent ev : events) {
						...
						if (ev.getEventMore().length() > 0) {
							...
							EventData ed = WebDataProcessor.processEventDataFromText(ev.getEventName(), text.toString());
							ev.setEventData(ed);
							
						}
						
						double e = (100 / channels) * evcur / evtot;
						double dPos = 100 * channelCounter / channels;
						double dSecPos = (e + dPos);
